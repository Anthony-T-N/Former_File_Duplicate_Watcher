...
secondary_directory_switch=false
...
while getopts ":p:s:d:" OPTION; do
    case $OPTION in
        ...
        s)
            ...
            secondary_directory_switch=true
            ;;
        d)
            re='^[0-9]+$'
            if ! [[ "$OPTARG" =~ $re ]] ; then
            ...
            ;;
        *)
            echo "Usage: $0 [-p directory_path] [-s secondary_directory_path] [-d directory_watch_depth]"
            exit 1
            ;;
    esac
done
...
echo "secondary_directory_switch: $secondary_directory_switch"
